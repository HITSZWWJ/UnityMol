...
namespace UMol
{
    public class UnityMolMain
    {
        ...
        {
            recordPythonCommand(true, command);
            Debug.Log("recordPythonCommand");
            if (GameObject.Find("Player").GetComponent<PlayerScript>().isRecipter)
            {
                Debug.Log("is recipter");
                GameObject.Find("Player").GetComponent<PlayerScript>().isRecipter = false;
                return;
            }
            else
            {
                Debug.Log("is not recipter");
                GameObject.Find("Player").GetComponent<PlayerScript>().isSender = true;
                GameObject.Find("Player").GetComponent<PlayerScript>().CmdExecuteCommand(command);
            }
        }
        public static void recordPythonCommand(bool flag, string command)
        {
